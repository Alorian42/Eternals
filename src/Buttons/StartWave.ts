...
export default class StartWaveButton {
  callback!: (this: StartWaveButton, playerIndex: number, button: Frame) => void;
  buttons: Array<Frame> = [];

  constructor(callback: (this: StartWaveButton, playerIndex: number, button: Frame) => void) {
  ...
  init(): void {
    for (let index = 0; index < bj_MAX_PLAYERS; index++) {
      ...
      BlzFrameSetAbsPoint(button.handle, FRAMEPOINT_CENTER, 0.03, 0.18 + (index / 100));
