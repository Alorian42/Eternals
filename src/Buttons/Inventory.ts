...
export default class InventoryButton {
  callback!: (this: InventoryButton, playerIndex: number, button: Frame) => void;
  buttons: Array<Frame> = [];

  constructor(callback: (this: InventoryButton, playerIndex: number, button: Frame) => void) {
  ...
  init(): void {
    for (let index = 0; index < bj_MAX_PLAYERS; index++) {
      const button = new Frame('InventoryButton', Frame.fromHandle(BlzGetOriginFrame(ORIGIN_FRAME_GAME_UI, 0)) as Frame, 0, index, 'BUTTON', 'ScoreScreenTabButtonTemplate');
      ...
      BlzFrameSetTexture(buttonIconFrame.handle, 'ReplaceableTextures\\CommandButtons\\BTNDust', 0, false);
      BlzFrameSetVisible(button.handle, GetPlayerId(GetLocalPlayer()) === index);
